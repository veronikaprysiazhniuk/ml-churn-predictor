...
def generate_demo_data():
    ...
    churn_prob=1/(1+np.exp(-(
        -2.0
        +0.8*(satisfaction<3).astype(float)
        +0.6*(last_activity_days>30).astype(float)
        +0.5*(contract_type=='Monthly').astype(float)
        -0.3*(tenure>24).astype(float)
        +0.4*(support_tickets>3).astype(float)
        -0.3*(plan=='Enterprise').astype(float)
        +0.3*(monthly_spend<40).astype(float)
...
if df is None:
    ...
    if model_type in["Random Forest","Both (compare)"]:
        ...
        models['Random Forest']={'model':rf,'pred':rf_pred,'prob':rf_prob}
    ...
    if model_type in["Logistic Regression","Both (compare)"]:
        ...
        lr_pred=lr.predict(X_test);lr_prob=lr.predict_proba(X_test)[:,1]
        models['Logistic Regression']={'model':lr,'pred':lr_pred,'prob':lr_prob}
    ...
    # Full predictions
    all_prob=best['model'].predict_proba(X_scaled)[:,1]
    ...
    # ─── EXEC SUMMARY ───
    st.markdown(f'<div class="es"><div class="es-label">Executive summary</div><div class="es-text">Current churn rate is <b>{churn_rate:.1f}%</b> across {total_customers:,} customers. The ML model identifies <b>{high_risk} high-risk customers</b> ({high_risk/total_customers*100:.0f}%) likely to leave within the next period. {"Estimated annual revenue at risk: <b>${revenue_at_risk:,.0f}</b>." if avg_spend>0 else ""} Model accuracy: <b>{best_results["AUC"]:.1%} AUC</b>.</div></div>',unsafe_allow_html=True)
